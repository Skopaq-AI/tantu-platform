...
if _HAS_PROM:
    READINGS_TOTAL = Counter("adapter_readings_total", "Total readings ingested", ["protocol", "adapter_id", "metric"])
    DEFECTS_TOTAL = Counter("adapter_defects_total", "Defect events emitted", ["protocol", "defect_class"])
    ...
    POLL_LATENCY = Histogram("adapter_poll_duration_seconds", "Poll duration", ["protocol", "adapter_id"])
else:
