...
def test_auth_required_for_register(client):
    r = client.post(
        "/adapters", json={"adapter_id": "x-1", "protocol": "opcua", "station_id": "s1"}
    )
    ...
    for proto, tag in [("camera", "camera"), ("opcua", "ns=2;i=1001")]:
        ...
        body = {
            ...
            "tags": [
                {
                    "source_tag": tag,
                    "metric": "gauge_value" if proto == "camera" else "pressure_bar",
                    "unit": "bar",
                }
            ],
            "params": {
            ...
            if proto == "camera"
            else {"endpoint": "opc.tcp://localhost:4840"},
            "poll_interval_ms": 0,
        }
        r = client.post("/adapters", json=body, headers=auth_header)
        assert r.status_code == 200, r.text
...
def test_tag_map_preview(client):
    r = client.post(
        "/tag-map/preview", json={"formula": "(a + b) / 2", "variables": {"a": 10, "b": 20}}
    )
    assert r.status_code == 200
    assert r.json()["value"] == pytest.approx(15.0)
    r = client.post(
        "/tag-map/preview", json={"formula": "__import__('os').system('x')", "variables": {}}
    )
