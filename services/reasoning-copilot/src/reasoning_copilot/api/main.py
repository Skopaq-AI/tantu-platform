...
async def lifespan(app: FastAPI):
    ...
    log.info("Reasoning Copilot ready — port %s qdrant=%s gemini=%s", settings.port, rag_store.backend, bool(settings.gemini_api_key))
    yield
...
@app.get("/prompts")
async def prompts():
    return {k: {"version": v.version, "description": v.description, "grounded": v.grounded, "require_citations": v.require_citations} for k, v in PROMPT_REGISTRY.items()}

# ---- auth helper (dev) -----------------------------------------------------

@app.post("/auth/token")
async def auth_token(sub: str = "operator-01", plant_id: str = "plant-demo-01", role: str = "operator"):
...
async def rag_ingest(body: RagIngestIn, request: Request, claims=Depends(optional_auth)):
    ...
    n = store.add(doc)
    return {"ingested": body.id, "chunks": n, "total_points": store.count(), "backend": store.backend}
...
async def rag_search(body: RagSearchIn, request: Request, claims=Depends(optional_auth)):
    ...
    return {
        "query": body.query,
        "hits": [{"doc_id": h.doc_id, "text": h.text[:400], "score": round(h.score, 4), "metadata": h.metadata} for h in hits],
...
async def rag_stats(claims=Depends(optional_auth)):
    store = get_store()
    return {"collection": store.collection, "count": store.count(), "backend": store.backend, "embedding_model": store.embedder.model_name, "dim": store.embedder.dim, "is_transformer": store.embedder.is_transformer}
...
async def ask(body: AskIn, request: Request, claims=Depends(optional_auth)):
    ...
    if claims and claims.get("plant_id") and claims["plant_id"] != body.plant_id:
        ...
        if claims.get("role") != "plant_admin":
            raise HTTPException(status_code=403, detail=f"plant_id mismatch: token {claims['plant_id']} vs request {body.plant_id}")
    ...
    if prompt_name != "ask_v1":
        variables = {"question": body.question, "plant_id": body.plant_id, "rag_context": rag_context, "lang": body.lang.value, "top_k": body.top_k}
        raw = await rt._route_generate(prompt_name, variables, rag_doc_ids, air_gapped=body.air_gapped)
        text = raw["text"]
        backend = raw["backend"]
        model = raw["model"]
        ...
        raw = await rt.answer(body.question, rag_context, rag_doc_ids, plant_id=body.plant_id, lang=body.lang.value, air_gapped=body.air_gapped, top_k=body.top_k)
        text = raw["text"]
        backend = raw["backend"]
        model = raw["model"]
        tokens_in = raw["tokens_in"]
...
async def correlate(body: CorrelateIn, request: Request, claims=Depends(optional_auth)):
    ...
    defect_tokens = " ".join([e.defect_class.value for e in body.events if e.defect_class.value != "none"])
    stations = " ".join([e.station_id for e in body.events])
    rag_query = f"{defect_tokens} {stations} {body.plant_id}".strip() or "pressure vibration thermal"
    ...
    if prompt_name != "correlate_v1":
        ...
        variables = {"events_json": _json.dumps(events_for_llm, default=str), "rag_context": rag_context, "lang": body.lang.value, "top_k": body.top_k}
        raw = await rt._route_generate(prompt_name, variables, rag_doc_ids, air_gapped=body.air_gapped)
    else:
        raw = await rt.correlate(events_for_llm, rag_context, rag_doc_ids, lang=body.lang.value, air_gapped=body.air_gapped, top_k=body.top_k)
